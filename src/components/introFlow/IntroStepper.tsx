 ...
 * the shape does both at a glance, and every available marker is tappable so
 * the flow can be walked in either direction.
 *
 * Each dot keeps a 44pt touch target while drawing at 8pt, so the row stays
 * visually quiet without becoming hard to hit. A first run keeps later
 * unavailable markers disabled until their setup prerequisite is ready.
 */
export function IntroStepper({
  ...
  return (
    <View accessibilityRole="tablist" style={styles.row} testID="intro-stepper">
      {Array.from({ length: count }, (_, step) => {
        const current = step === index;
        const disabled = step > maxReachableIndex;
