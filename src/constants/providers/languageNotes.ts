...
const NATIVE_TTS_LANGUAGE_NOTES_BY_LANGUAGE: LocalizedResource<string> = {
  ...
  ar:
    "يعتمد دعم اللغات على أصوات النظام المثبتة على الجهاز. تختلف القائمة الدقيقة وجودة النطق والتوفر دون اتصال باختلاف نظام التشغيل والجهاز.",
};
...
const BRAZILIAN_PORTUGUESE_PROVIDER_API_KEY_PLACEHOLDER_OVERRIDES: Partial<
  Record<Provider, string>
> = {
  mistral: "Insira a chave da API",
};

const RUSSIAN_PROVIDER_API_KEY_PLACEHOLDER_OVERRIDES: Partial<
  Record<Provider, string>
> = {
...
const ARABIC_PROVIDER_API_KEY_PLACEHOLDER_OVERRIDES: Partial<
  Record<Provider, string>
> = {
...
> = {
  ...
  "pt-BR": Object.fromEntries(
    PROVIDER_ORDER.map((provider) => [
      provider,
      BRAZILIAN_PORTUGUESE_PROVIDER_API_KEY_PLACEHOLDER_OVERRIDES[provider] ??
        PROVIDER_CONFIGS[provider].apiKeyPlaceholder,
    ]),
  ) as Record<Provider, string>,
  ru: Object.fromEntries(
    PROVIDER_ORDER.map((provider) => [
      provider,
      RUSSIAN_PROVIDER_API_KEY_PLACEHOLDER_OVERRIDES[provider] ??
        PROVIDER_CONFIGS[provider].apiKeyPlaceholder,
    ]),
  ) as Record<Provider, string>,
  ...
  ar: Object.fromEntries(
    PROVIDER_ORDER.map((provider) => [
      provider,
      ARABIC_PROVIDER_API_KEY_PLACEHOLDER_OVERRIDES[provider] ??
        PROVIDER_CONFIGS[provider].apiKeyPlaceholder,
    ]),
  ) as Record<Provider, string>,
};
...
> = {
  ...
  "pt-BR": {
    openai:
      "A OpenAI oferece atualmente gpt-4o-mini-tts, tts-1 e tts-1-hd para síntese de fala. A OpenAI não publica para TTS uma lista compacta de idiomas com bom suporte como a de STT e informa que as vozes são otimizadas para inglês.",
    gemini:
      "O Gemini TTS oferece suporte atualmente a árabe, bengali, holandês, inglês, francês, alemão, hindi, indonésio, italiano, japonês, coreano, chinês mandarim, polonês, português, romeno, russo, espanhol, tâmil, telugu, tailandês, turco, ucraniano, urdu e vietnamita.",
    xai:
      "O xAI TTS oferece suporte atualmente a árabe, holandês, inglês, francês, alemão, hindi, indonésio, italiano, japonês, coreano, polonês, português, russo, espanhol, tailandês, turco, vietnamita e chinês.",
  },
  ru: {
    openai:
      "OpenAI сейчас предлагает gpt-4o-mini-tts, tts-1 и tts-1-hd для синтеза речи. OpenAI не публикует для TTS такой же компактный список хорошо поддерживаемых языков, как для STT, и отмечает, что голоса оптимизированы для английского.",
    gemini:
  ...
  ar: {
    openai:
      "تقدم OpenAI حاليًا gpt-4o-mini-tts وtts-1 وtts-1-hd لتحويل النص إلى كلام. لا تنشر OpenAI قائمة موجزة للغات ذات الدعم الجيد في TTS مثل STT، وتشير إلى أن الأصوات محسّنة للإنجليزية.",
    gemini:
      "يدعم Gemini TTS حاليًا العربية والبنغالية والهولندية والإنجليزية والفرنسية والألمانية والهندية والإندونيسية والإيطالية واليابانية والكورية والصينية المندرينية والبولندية والبرتغالية والرومانية والروسية والإسبانية والتاميلية والتيلوغوية والتايلاندية والتركية والأوكرانية والأردية والفيتنامية.",
    xai:
      "يدعم xAI TTS حاليًا العربية والهولندية والإنجليزية والفرنسية والألمانية والهندية والإندونيسية والإيطالية واليابانية والكورية والبولندية والبرتغالية والروسية والإسبانية والتايلاندية والتركية والفيتنامية والصينية.",
  },
};
...
const SPEECH_NOTE_FORMATTERS: LocalizedResource<SpeechNoteFormatter> = {
  ...
  "pt-BR": {
    voicesAcross: (voices, languages) =>
      `${voices} vozes em ${languages} idiomas`,
    supportsLanguages: (languages) => `Oferece suporte a ${languages} idiomas`,
    multilingual: "Multilíngue",
    englishOptimized: "As vozes são otimizadas para inglês",
    duration: (value, unit) => {
      const units = {
        hour: value === 1 ? "hora" : "horas",
        minute: value === 1 ? "minuto" : "minutos",
    ...
    fileUploadUpTo: (limit) => `Upload de arquivo de até ${limit}`,
    approximateFileUpload: (limit) =>
      `Limite aproximado de upload de arquivo: ${limit}`,
    approximateFileUploadRange: (minimum, maximum) =>
      `Limite aproximado de upload de arquivo de ${minimum} a ${maximum}, dependendo do plano`,
    audioUpTo: (limit) => `Áudio de até ${limit}`,
  },
  ru: {
    voicesAcross: (voices, languages) =>
      `${voices} голосов для ${languages} языков`,
    supportsLanguages: (languages) => `Поддерживает языков: ${languages}`,
    multilingual: "Многоязычный",
    englishOptimized: "Голоса оптимизированы для английского",
    duration: (value, unit) =>
      `${value} ${{ hour: "ч", minute: "мин", second: "с" }[unit]}`,
    fileUploadUpTo: (limit) => `Загрузка файлов до ${limit}`,
    approximateFileUpload: (limit) =>
      `Примерный предел размера файла: ${limit}`,
    approximateFileUploadRange: (minimum, maximum) =>
  ...
  ar: {
    voicesAcross: (voices, languages) =>
      `${voices} صوتًا عبر ${languages} لغة`,
    supportsLanguages: (languages) => `يدعم ${languages} لغة`,
    multilingual: "متعدد اللغات",
    englishOptimized: "الأصوات محسّنة للإنجليزية",
    duration: (value, unit) => {
      const units = {
        hour: "ساعة",
        minute: "دقيقة",
    ...
    fileUploadUpTo: (limit) => `رفع ملفات حتى ${limit}`,
    approximateFileUpload: (limit) =>
      `الحد التقريبي لرفع الملفات هو ${limit}`,
    approximateFileUploadRange: (minimum, maximum) =>
      `يتراوح الحد التقريبي لرفع الملفات من ${minimum} إلى ${maximum} حسب الخطة`,
    audioUpTo: (limit) => `صوت حتى ${limit}`,
  },
};
