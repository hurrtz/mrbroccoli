...
export const ru = {
  ...
  ulraModeInfo:
    "Суперрежим сначала отдельно опрашивает каждую готовую модель с главного экрана. Затем в каждом раунде все модели изучают все предыдущие ответы. Выбранная модель формирует итоговый ответ. Материалы обсуждения передаются всем задействованным провайдерам.",
  ...
  runtimeCompatibilityOverridesDescription: ({ count }) =>
    `${count} конфигураций моделей или настроек, недоступность которых подтвердил провайдер, отключены только на этом устройстве. Мистер Брокколи автоматически их обходит.`,
