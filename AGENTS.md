...
- `src/components/IntroBanner.tsx` and `src/components/introFlow/` are the
  first-run introduction: a dark banner over the workspace opening a
  ...
  `src/components/autoSetup/`. `introTheme.ts` holds its palette, deliberately independent
...
- If response-mode behavior changes, the following files usually need to stay in sync:
  ...
  - `src/screens/main/MainScreenRouteByline.tsx`
...
- Home-screen interaction changes usually belong in `src/screens/MainScreen.tsx`, `src/screens/main/MainScreenWorkspace.tsx`, and the orb-workspace components under `src/design-system/`.
