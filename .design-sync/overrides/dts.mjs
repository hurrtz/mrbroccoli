// forked from design-sync lib/dts.mjs — filters React Native base props (View/Pressable/Text) the way the bundled copy filters @types/react, so each component's own API is not buried under 60+ inherited native props
...
// cross-platform.
// FORK: the trailing group is added for React Native — structural, visual-state,
// and accessibility props that stay meaningful once the rest of the native base
// is filtered in isOwnProp below.
const KEEP_PROP = /^(children|className|style|as|asChild|ref|id|disabled|onPress|onLongPress|numberOfLines|placeholder|value|onChangeText|accessible|accessibilityLabel|accessibilityRole|accessibilityState|accessibilityHint)$/;
...
function isOwnProp(p, pkgDir, styleSystemDirs) {
  ...
  // FORK: React Native's View/Pressable/Text bases contribute 60+ props per
  // component — Apple TV parallax, Android ripple, native-only a11y plumbing.
  // They drown the component's real API exactly as a CSS-in-JS style-system
  // base does, and their types (ViewStyle, Insets, AccessibilityState) are not
  // importable in the emitted .d.ts, so leaving them in also breaks parsing.
  // The structurally meaningful ones are kept by KEEP_PROP above.
  if (/\/node_modules\/react-native(-web)?\//.test(fp)) return false;
