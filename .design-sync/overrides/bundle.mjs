// forked from design-sync lib/bundle.mjs — adds resolveExtensions so React Native platform variants (.web.js) win, as Metro does
...
// FORK: see the plugin's own header — .svg imports are React components here.
import { svgComponentPlugin } from '../shims/svg-loader.mjs';
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  const plugins = [reactShim, svgComponentPlugin];
  if (pathsPlugin) plugins.unshift(pathsPlugin);
  return {
    bundle: true,
    // React Native ships one module per platform and lets the bundler pick:
    // `import './findNodeHandle'` means findNodeHandle.web.js in a browser and
    // findNodeHandle.js on a device. esbuild has no platform-extension
    // resolution, so without this it loads the native file and then fails
    // parsing react-native's Flow-typed source. This is the standard
    // react-native-web resolver order.
    ...
    minify: false,
    // FORK: React Native and Expo expect two globals a browser bundle has to
    // supply, or the IIFE throws ReferenceError on load and assigns nothing to
    // window.<GLOBAL> — leaving every component reported as missing.
    //
    //   __DEV__       a React Native global guarding dev-only code paths
    //                 throughout react-native-web, reanimated, and
    //                 gesture-handler. lib/previews.mjs already defines it for
    //                 the preview pass; the runtime bundle needs it too.
    //   process.env   expo modules read process.env.EXPO_OS to branch on
    //                 platform, plus NODE_DEBUG/JEST_WORKER_ID. Defining only
    //                 process.env.NODE_ENV (as upstream does) substitutes that
    //                 one expression and leaves `process` itself unbound.
    //                 "web" is the honest value here: this bundle runs in a
    //                 browser.
    //   global        React Native's JS runtime exposes Node's `global`;
    //                 in a browser that name is `globalThis`.
    define: {
      global: 'globalThis',
      'process.env.NODE_ENV': '"development"',
      'process.env.EXPO_OS': '"web"',
      'process.env': '{"NODE_ENV":"development","EXPO_OS":"web"}',
      process: '{"env":{"NODE_ENV":"development","EXPO_OS":"web"},"platform":"web"}',
      __DEV__: 'true',
    },
  };
}
