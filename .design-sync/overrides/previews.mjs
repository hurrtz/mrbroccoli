// forked from design-sync lib/previews.mjs — adds resolveExtensions so React Native platform variants (.web.js) win, as Metro does
...
export async function buildPreviews({ components, previewDir, genDir, OUT, reactShim, NODE_MODULES, pathsPlugin, importPlugins, loaders }) {
  ...
  const plugins = [reactShim, svgComponentPlugin, ...(importPlugins ?? []), ...(pathsPlugin ? [pathsPlugin] : [])];
  for (const c of components) {
