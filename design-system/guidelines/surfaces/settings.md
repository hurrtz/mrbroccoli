...
- **Editions change contents, never structure.** Both editions see the same seven pages in the same order. Free renders provider routes as `locked` ghost rows — quietly, with no upsell band or sheet (owner call, 2026-08: `PremiumBand` and `PremiumUpgradeModal` are retired; a locked row and the editions row in App & diagnostics are the whole story); system and on-device routes work fully. Premium unlocks sections in place — the map the user learned stays valid.
