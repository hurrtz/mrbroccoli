...
export function IconAction({ icon, label, danger = false, spin = false, onPress }) {
  return (
    <span role="button" aria-label={label} onClick={(e) => { e.stopPropagation(); if (onPress) onPress(); }}
      ...
      <span style={{ width: 36, height: 36, borderRadius: "var(--mb-radius-icon-button)", display: "inline-flex", alignItems: "center", justifyContent: "center", border: "1px solid " + (danger ? "var(--mb-color-danger)" : "var(--mb-color-border-strong)"), background: "var(--mb-color-surface)" }}>
