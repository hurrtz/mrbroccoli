...
function verifyNativeLicenses() {
  ...
  if (
    !modelLicenseCsv.includes(
      "kokoro-int8-multi-lang-v1_1.tar.bz2,apache-2.0,yes,high,archive_license_file",
    )
  ) {
    fail("Kokoro's reviewed Apache-2.0 model license record changed");
  }
...
function renderNotices(packages) {
  ...
  const sections = [...groups.values()]
    .map((group) => ({
      ...group,
      packages: group.packages.sort((left, right) =>
  ...
  return [
    ...
    "",
    "The optional Kokoro model is not bundled. Its downloaded archive declares",
    "Apache-2.0 and includes `kokoro-int8-multi-lang-v1_1/LICENSE`.",
    "",
    "## JavaScript and React Native packages",
    "",
    ...sections,
    "## Native Kokoro runtime",
    "",
    ...nativeSections,
    "",
  ].join("\n");
  ...
  if (failures.length > 0) {
    fail(
      [
        ...
        ...failures.map(
          (entry) => `- ${entry.package}: ${entry.license}`,
        ),
      ].join("\n"),
    );
  }
